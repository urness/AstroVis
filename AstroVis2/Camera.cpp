...
void Camera::transform(int cameraMode, float stereoDistance) {
   ...
   // if we're in a stereo mode, figure out the fudge factor for the frustum
   if ( cameraMode != CAMERA_MONO ) {
       Vector3 direction = _target - _position;
       right = (direction ^ _up).normalized();
       offset = 0.5 * (_near_plane / direction.length() ) * (stereoDistance) * orientationMultiplier;
   }

   glFrustum(
      - 0.5 * _viewport_width + offset,  0.5 * _viewport_width + offset,	// left, right
